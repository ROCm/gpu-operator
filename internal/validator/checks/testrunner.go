...
// ValidateTestRunner validates the test runner component
func ValidateTestRunner(ctx context.Context, k8sClient client.Client, devConfig *gpuev1alpha1.DeviceConfig) gpuev1alpha1.ComponentValidationResult {
	...
	dsName := fmt.Sprintf("%s-test-runner-daemonset", devConfig.Name)
	ds := &appsv1.DaemonSet{}
	...
	if err != nil {
		...
		result.Checks = append(result.Checks, gpuev1alpha1.ValidationCheck{
			...
			Passed:  false,
			Message: "Test Runner DaemonSet not found",
		})
		return result
	}
	...
	result.Checks = append(result.Checks, gpuev1alpha1.ValidationCheck{
		...
		Name:    "DaemonSet exists",
		Passed:  true,
		Message: "Test Runner DaemonSet found",
	})
	...
	if devConfig.Spec.TestRunner.Image != "" {
		if len(ds.Spec.Template.Spec.Containers) > 0 {
			actualImage := ds.Spec.Template.Spec.Containers[0].Image
			expectedImage := devConfig.Spec.TestRunner.Image

			if actualImage != expectedImage {
				result.Status = "degraded"
				result.Checks = append(result.Checks, gpuev1alpha1.ValidationCheck{
					Type:          "ConfigurationMatch",
					Name:          "Test Runner image matches spec",
					Passed:        false,
	...
	if devConfig.Spec.TestRunner.ImagePullPolicy != "" {
		if len(ds.Spec.Template.Spec.Containers) > 0 {
			actualPolicy := string(ds.Spec.Template.Spec.Containers[0].ImagePullPolicy)
			expectedPolicy := devConfig.Spec.TestRunner.ImagePullPolicy
	...
	labelSelector := map[string]string{
		"app": fmt.Sprintf("%s-test-runner", devConfig.Name),
	}

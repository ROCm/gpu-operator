...
// ValidateDevicePlugin validates the device plugin component
func ValidateDevicePlugin(ctx context.Context, k8sClient client.Client, devConfig *gpuev1alpha1.DeviceConfig) gpuev1alpha1.ComponentValidationResult {
	...
	dsName := fmt.Sprintf("%s-device-plugin-daemonset", devConfig.Name)
	ds := &appsv1.DaemonSet{}
	...
	if err != nil {
		...
		result.Checks = append(result.Checks, gpuev1alpha1.ValidationCheck{
			...
			Passed:  false,
			Message: "Device Plugin DaemonSet not found",
	...
	result.Checks = append(result.Checks, gpuev1alpha1.ValidationCheck{
		...
		Name:    "DaemonSet exists",
		Passed:  true,
		Message: "Device Plugin DaemonSet found",
	})
	...
	if devConfig.Spec.DevicePlugin.DevicePluginImage != "" {
		// Get actual image from DaemonSet
		if len(ds.Spec.Template.Spec.Containers) > 0 {
			actualImage := ds.Spec.Template.Spec.Containers[0].Image
			expectedImage := devConfig.Spec.DevicePlugin.DevicePluginImage

			if actualImage != expectedImage {
				result.Status = "degraded"
				result.Checks = append(result.Checks, gpuev1alpha1.ValidationCheck{
					Type:          "ConfigurationMatch",
					Name:          "Device Plugin image matches spec",
					Passed:        false,
	...
	if devConfig.Spec.DevicePlugin.DevicePluginImagePullPolicy != "" {
		if len(ds.Spec.Template.Spec.Containers) > 0 {
			...
			expectedPolicy := devConfig.Spec.DevicePlugin.DevicePluginImagePullPolicy
	...
	labelSelector := map[string]string{
		"app": fmt.Sprintf("%s-device-plugin", devConfig.Name),
	}

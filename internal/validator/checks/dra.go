...
// ValidateDRADriver validates the DRA driver component
func ValidateDRADriver(ctx context.Context, k8sClient client.Client, devConfig *gpuev1alpha1.DeviceConfig) gpuev1alpha1.ComponentValidationResult {
	...
	dsName := fmt.Sprintf("%s-dra-driver-daemonset", devConfig.Name)
	ds := &appsv1.DaemonSet{}
	...
	if err != nil {
		...
		result.Checks = append(result.Checks, gpuev1alpha1.ValidationCheck{
			...
			Passed:  false,
			Message: "DRA Driver DaemonSet not found",
	...
	result.Checks = append(result.Checks, gpuev1alpha1.ValidationCheck{
		...
		Name:    "DaemonSet exists",
		Passed:  true,
		Message: "DRA Driver DaemonSet found",
	})
	...
	if devConfig.Spec.DRADriver.Image != "" {
		if len(ds.Spec.Template.Spec.Containers) > 0 {
			actualImage := ds.Spec.Template.Spec.Containers[0].Image
			expectedImage := devConfig.Spec.DRADriver.Image

			if actualImage != expectedImage {
				result.Status = "degraded"
				result.Checks = append(result.Checks, gpuev1alpha1.ValidationCheck{
					Type:          "ConfigurationMatch",
					Name:          "DRA Driver image matches spec",
					Passed:        false,
	...
	if devConfig.Spec.DRADriver.ImagePullPolicy != "" {
		if len(ds.Spec.Template.Spec.Containers) > 0 {
			...
			expectedPolicy := devConfig.Spec.DRADriver.ImagePullPolicy
	...
	labelSelector := map[string]string{
		"app": fmt.Sprintf("%s-dra-driver", devConfig.Name),
	}

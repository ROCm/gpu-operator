...
// ValidateConfigManager validates the config manager component
func ValidateConfigManager(ctx context.Context, k8sClient client.Client, devConfig *gpuev1alpha1.DeviceConfig) gpuev1alpha1.ComponentValidationResult {
	...
	dsName := fmt.Sprintf("%s-config-manager-daemonset", devConfig.Name)
	ds := &appsv1.DaemonSet{}
	...
	if err != nil {
		...
		result.Checks = append(result.Checks, gpuev1alpha1.ValidationCheck{
			...
			Passed:  false,
			Message: "Config Manager DaemonSet not found",
		})
		return result
	}
	...
	result.Checks = append(result.Checks, gpuev1alpha1.ValidationCheck{
		...
		Name:    "DaemonSet exists",
		Passed:  true,
		Message: "Config Manager DaemonSet found",
	})
	...
	if devConfig.Spec.ConfigManager.Image != "" {
		if len(ds.Spec.Template.Spec.Containers) > 0 {
			actualImage := ds.Spec.Template.Spec.Containers[0].Image
			expectedImage := devConfig.Spec.ConfigManager.Image

			if actualImage != expectedImage {
				result.Status = "degraded"
				result.Checks = append(result.Checks, gpuev1alpha1.ValidationCheck{
					Type:          "ConfigurationMatch",
					Name:          "Config Manager image matches spec",
					Passed:        false,
	...
	if devConfig.Spec.ConfigManager.ImagePullPolicy != "" {
		if len(ds.Spec.Template.Spec.Containers) > 0 {
			actualPolicy := string(ds.Spec.Template.Spec.Containers[0].ImagePullPolicy)
			expectedPolicy := devConfig.Spec.ConfigManager.ImagePullPolicy
	...
	labelSelector := map[string]string{
		"app": fmt.Sprintf("%s-config-manager", devConfig.Name),
	}

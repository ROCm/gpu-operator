...
type ConditionWorkflowMapping struct {
	...
	ValidationTestsProfile string `json:"validationTestsProfile" yaml:"validationTestsProfile"`
...
func (h *remediationMgrHelper) createDefaultConfigMap(ctx context.Context, name string, namespace string) (*v1.ConfigMap, error) {
	...
	workflowYaml := `- nodeCondition: "GPU_Temp_Exceeded"
  workflowTemplate: "default-template"
  validationTestsProfile: "test-profile-1"`
...
NODE_NAME=$(kubectl get pod $(hostname) -n {{workflow.namespace}} -o jsonpath='{.spec.nodeName}')

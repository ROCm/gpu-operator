...
| deviceConfig.spec.remediationWorkflow.testerImage | string | `"registry.test.pensando.io:5000/test-runner:latest"` | Container image used for testing during remediation |
...
| remediation.controller | object | `{"affinity":{"nodeAffinity":{"preferredDuringSchedulingIgnoredDuringExecution":[{"preference":{"matchExpressions":[{"key":"node-role.kubernetes.io/control-plane","operator":"Exists"}]},"weight":1}]}},"image":"quay.io/argoproj/workflow-controller:v4.0.3","nodeSelector":{}}` | Set the controller image for remediation workflow controller deployment |
